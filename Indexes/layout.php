<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 3.2 Final//EN">
<html>
	<head>
		<meta charset="utf-8">
		<meta name="viewport" content="width=device-width, initial-scale=1.0,maximum-scale=1.0, user-scalable=no"/>
		<title>Index of <?php echo urldecode($path);?></title>
		<style>
img[alt=file]{
	background-image: url("data:image/gif;base64,");
}
img[alt=DIR]{
	background-image: url("data:image/gif;base64,");
}
img[alt=compressed]{
	background-image: url("data:image/gif;base64,");
}
img[alt=binary]{
	background-image: url("data:image/gif;base64,");
}
img[alt=pdf]{
	background-image: url("data:image/gif;base64,");
}
img[alt=C]{
	background-image: url("data:image/gif;base64,");
}
img[alt=CPP]{
	background-image: url("data:image/gif;base64,R0lGODlhFAAWAKIEAJmZmTMzMwAAAP///////wAAAAAAAAAAACH5BAEAAAQALAAAAAAUABYAAANnGLq89JCEQaudIb5pO88RNwjkVYEbRa7nBXFlqQYCTaejkFdrj8udAWAIEPx0Mt2IaJSokNCesAmTjlSUIi6nDE633o7Wye2xstSwd1w1n79ktYX9dAPp8nk6X2EHiICBQzcQdoZNCQA7");
}
img[alt=CSharp]{
	background-image: url("data:image/gif;base64,R0lGODlhFAAWAKIEAJmZmTMzMwAAAP///////wAAAAAAAAAAACH5BAEAAAQALAAAAAAUABYAAANlGLq89JCEQaudIb5pO88RNwjkVYEbRa7nBXFlqQYCTaejkFdrj8u8EWAIEPx0sR7RKFEhdaMopXhk9aDUphMa5WZFnfDgu70CyWIx2tyroNOdNfvMBMOn9Xu8HiD6/0M3EHNzDwkAOw==");
}
img[alt=style],img[alt=text],img[alt=JSON]{
	background-image: url("data:image/gif;base64,");
}
img[alt=tar]{
	background-image: url("data:image/gif;base64,");
}
img[alt=script]{
	background-image: url("data:image/gif;base64,");
}
img[alt=HTML],img[alt=word],img[alt=powerpoint],img[alt=excel]{
	background-image: url("data:image/gif;base64,");
}
img[alt=font]{
	background-image: url("data:image/gif;base64,");
}
img[alt=php]{
	background-image: url("data:image/gif;base64,");
}
img[alt=sound]{
	background-image: url("data:image/gif;base64,");
}
img[alt=movie]{
	background-image: url("data:image/gif;base64,");
}
img[alt=image]{
	background-image: url("data:image/gif;base64,");
}
		</style>
	</head>
	<body>
		<?php view::section('content');?>
	</body>
</html>